...
export default function SettingsPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 p-6">
        ...
        <section className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4">
          {[1, 2, 3, 4].map((item) => (
            <fieldset
              key={item}
              className="max-w-sm animate-pulse rounded-lg border-4 border-gray-300 p-2 text-xl"
            >
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
          ...
          <div className="mb-4 grid grid-cols-3 gap-4">
            <div className="rounded-lg bg-slate-900/50 p-3">
              ...
              <p className="text-lg font-semibold text-white">
                {topBooks.length > 0
                  ? topBooks.reduce((sum, book) => sum + (book.count || 0), 0)
                  : 0}
              </p>
            ...
            <div className="rounded-lg bg-slate-900/50 p-3">
              ...
              <p className="text-lg font-semibold text-white">
                {topJournals.length > 0
                  ? topJournals.reduce(
                      (sum, journal) => sum + (journal.count || 0),
                      0,
                    )
                  : 0}
              </p>
                ...
                if (currentData.length > 0) {
                  return currentData.slice(0, 7).map((item, index) => {
                    const maxCount = Math.max(
                      ...currentData.map((d) => d.count || 0),
                    );
                    const heightPercentage =
                      maxCount > 0 ? ((item.count || 0) / maxCount) * 100 : 20;
                    const normalizedHeight = Math.max(
                      20,
                      Math.min(95, heightPercentage),
                    );

                    return (
                      <div
                        key={item._id}
                        className={`w-3 rounded-sm ${colorClass}/30 group relative`}
                      ...
                      >
                        ...
                        <div className="absolute bottom-full left-1/2 z-10 mb-2 -translate-x-1/2 transform rounded bg-black px-2 py-1 text-xs whitespace-nowrap text-white opacity-0 transition-opacity group-hover:opacity-100">
                          {item.judul.length > 20
                            ? `${item.judul.substring(0, 20)}...`
                            : item.judul}
                          <br />
                          Count: {item.count || 0}
                        </div>
      ...
      <section className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4">
        <fieldset className="w-full max-w-sm rounded-lg border-4 border-green-500 p-2 text-xl">
          <legend className="px-2 text-xl font-semibold underline decoration-green-500/60 decoration-2">
            Top 5 Most Borrowed Books
          </legend>
          <div className="text-md flex flex-col gap-2 px-2 font-serif">
            {topBooks.length > 0 ? (
        ...
        <fieldset className="w-full max-w-sm rounded-lg border-4 border-blue-500 p-2 text-xl">
        ...
        <fieldset className="w-full max-w-sm rounded-lg border-4 border-purple-500 p-2 text-xl">
        ...
        <fieldset className="w-full max-w-sm rounded-lg border-4 border-orange-500 p-2 text-xl">
