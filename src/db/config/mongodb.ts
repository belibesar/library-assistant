    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('ENOTFOUND') || error.message.includes('ETIMEOUT')) {
          throw new Error('Cannot connect to MongoDB Atlas. Please check your internet connection and MongoDB URI. Try using a VPN or different network.');
        }
        ...
        if (error.message.includes('queryTxt')) {
          throw new Error('DNS resolution failed for MongoDB Atlas. Please try again or check your network connection.');
        }
      }
